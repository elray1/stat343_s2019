---
title: 'Stat 343: Motivation for Numerical Optimization of the Likelihood'
author: "Evan Ray"
output:
  pdf_document:
    fig_height: 2.7
...
For our data set, the maximum likelihood estimates are approximately $\hat{\alpha}_{MLE} = 0.440813$ and $\hat{\beta}_{MLE} = 1.964378$

```{r}
ggplot(data = il_storms, mapping = aes(x = precipitation)) +
  geom_histogram(center = 0.005, binwidth = 0.01, mapping = aes(y = ..density..)) +
  stat_function(fun = dgamma, args = list(shape = 0.440813, rate = 1.964378), color = "orange") +
  theme_bw()
```

\newpage

## Maximum Likelihood for a Gamma($\alpha$, $\beta$)
