...
In problem 2 of problem set 3, we analyzed data from experiments that were conducted to learn about the spatial organization of chromosomes.  We had $n$ measurements of the distances between pairs of small DNA sequences, which we modeled as $X_i \simiid \text{Rayleigh}(\theta)$.

In that problem set, we found the maximum likelihood estimator and estimate of $\theta$; now, we will find an approximate confidence interval for $\theta$.
